...
from hummingbot.connector.exchange.bitblinx.bitblinx_order_book_tracker import BitblinxOrderBookTracker
from hummingbot.connector.exchange.bitblinx.bitblinx_api_order_book_data_source import BitblinxAPIOrderBookDataSource
from hummingbot.core.data_type.order_book import OrderBook


class BitblinxOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[BitblinxOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: BitblinxOrderBookTracker = BitblinxOrderBookTracker(cls.trading_pairs)
    ...
    def test_order_book_trade_event_emission(self):
        ...
        for ob_trade_event in self.event_logger.event_log:
            ...
            print(type(ob_trade_event.timestamp))
    ...
    def test_tracker_integrity(self):
        ...
        print(self.order_book_tracker.ready)
        print("ETH LAST: {0}".format(eth_usdt.snapshot)) 
    #    self.assertIsNot(eth_usdt.last_diff_uid, 0)
        self.assertGreaterEqual(eth_usdt.get_price_for_volume(True, 3).result_price,
                                eth_usdt.get_price(True))
        self.assertLessEqual(eth_usdt.get_price_for_volume(False, 3).result_price,
                             eth_usdt.get_price(False))

    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            BitblinxAPIOrderBookDataSource.get_last_traded_prices(["BTC-USDT"]))
