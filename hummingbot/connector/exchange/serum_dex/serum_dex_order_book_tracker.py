...
from hummingbot.connector.exchange.serum_dex.serum_dex_active_order_tracker import \
    SerumDexActiveOrderTracker
from hummingbot.connector.exchange.serum_dex.serum_dex_order_book import SerumDexOrderBook
from hummingbot.connector.exchange.serum_dex.serum_dex_order_book_message import \
    SerumDexOrderBookMessage
from hummingbot.connector.exchange.serum_dex.serum_dex_order_book_tracker_entry import \
    SerumDexOrderBookTrackerEntry
from .serum_dex_api_order_book_data_source import SerumDexOrderBookDataSource
...
TRACKER_TYPE = Dict[str, SerumDexOrderBookTrackerEntry]


class SerumDexOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: List[str]):
        super().__init__(
            SerumDexOrderBookDataSource(trading_pairs),
            trading_pairs
        )
        ...
        self._active_order_trackers: TRACKER_TYPE = defaultdict(SerumDexActiveOrderTracker)
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: SerumDexOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: SerumDexOrderBook = self._order_books[trading_pair]
