...
from .ftx_perpetual_auth import FtxPerpetualAuth
from .ftx_perpetual_websocket import FtxPerpetualWebsocket


class FtxPerpetualAPIUserStreamDataSource(UserStreamTrackerDataSource):
    MAX_RETRIES = 20
    ...
    def __init__(self, ftx_perpetual_auth: FtxPerpetualAuth, trading_pairs: Optional[List[str]] = []):
    ...
    async def _listen_to_orders_trades_balances(self) -> AsyncIterable[Any]:
        ...
        try:
            ws = FtxPerpetualWebsocket(self._ftx_perpetual_auth)
            await ws.connect()
