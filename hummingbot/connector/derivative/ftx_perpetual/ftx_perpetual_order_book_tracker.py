...
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_order_book_message import \
    FtxPerpetualOrderBookMessage
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_order_book_tracker_entry import \
    FtxPerpetualOrderBookTrackerEntry
from .ftx_perpetual_api_order_book_data_source import FtxPerpetualAPIOrderBookDataSource
...
TRACKER_TYPE = Dict[str, FtxPerpetualOrderBookTrackerEntry]


class FtxPerpetualOrderBookTracker(OrderBookTracker):
    ...
    def __init__(self, trading_pairs: List[str]):
        super().__init__(
            FtxPerpetualAPIOrderBookDataSource(trading_pairs),
            trading_pairs
        )
    ...
    async def _order_book_diff_router(self):
        ...
        while True:
            try:
                ob_message: FtxPerpetualOrderBookMessage = await self._order_book_diff_stream.get()
                ...
                order_book: FtxPerpetualOrderBook = self._order_books[trading_pair]
