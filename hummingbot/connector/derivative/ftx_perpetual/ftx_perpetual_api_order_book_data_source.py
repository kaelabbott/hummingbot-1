...
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_websocket import FtxPerpetualWebsocket
from hummingbot.connector.derivative.ftx_perpetual.ftx_perpetual_utils import convert_to_exchange_trading_pair, timestamp_to_int
...
class FtxPerpetualAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_price(cls, trading_pair: str) -> float:
        async with aiohttp.ClientSession() as client:
            resp = await client.get(f"{FTX_MARKET}/{convert_to_exchange_trading_pair(trading_pair)}")
    ...
    async def get_snapshot(client: aiohttp.ClientSession, trading_pair: str, limit: int = 1000) -> Dict[str, Any]:
        async with client.get(f"{FTX_MARKET}/{convert_to_exchange_trading_pair(trading_pair)}/orderbook?depth=100", ) as response:
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        async with aiohttp.ClientSession() as client:
            ...
            snapshot_msg: OrderBookMessage = FtxPerpetualOrderBook.snapshot_message_from_exchange(
                snapshot,
                snapshot_timestamp,
                metadata={"trading_pair": convert_to_exchange_trading_pair(trading_pair)}
            )
        ...
        while True:
            try:
                ...
                for trading_pair in trading_pairs:
                    ws = FtxPerpetualWebsocket()
                    logging.getLogger('asyncio').setLevel(logging.ERROR)
                    logging.getLogger('asyncio.coroutines').setLevel(logging.ERROR)
                    logging.getLogger('websockets.server').setLevel(logging.ERROR)
                    logging.getLogger('websockets.protocol').setLevel(logging.ERROR)
                    await ws.connect()
                    await ws.subscribe({'channel': 'trades', 'market': convert_to_exchange_trading_pair(trading_pair)})
                    async for response in ws.on_message_public():
                        if response.get("data") is None:
                            continue
                        for trade in response["data"]:
                            trade['market'] = convert_to_exchange_trading_pair(trading_pair)
                            trade: Dict[Any] = trade
                            trade_timestamp = timestamp_to_int(trade["time"])
                            trade_msg: OrderBookMessage = FtxPerpetualOrderBook.trade_message_from_exchange(
                                trade,
        ...
        while True:
            try:
                ...
                for trading_pair in trading_pairs:
                    ...
                    logging.getLogger('websockets.protocol').setLevel(logging.ERROR)
                    await ws.connect()
                    await ws.subscribe({'channel': 'orderbook', 'market': convert_to_exchange_trading_pair(trading_pair)})
                    async for response in ws.on_message_public():
                        if response.get("data") is None:
                            continue
                        ...
                        if snapshot:
                            snapshot_timestamp: float = time.time()
                            snapshot_msg: OrderBookMessage = FtxPerpetualOrderBook.diff_message_from_exchange(
                                snapshot,
                                snapshot_timestamp,
                            )
